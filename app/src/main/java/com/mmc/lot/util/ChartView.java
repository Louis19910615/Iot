...
public class ChartView extends View {
    ...
    //绘制XY轴坐标对应的画笔
    private Paint xyPaint, xyPaint1;
    //绘制XY轴的文本对应的画笔
    private Paint xyTextPaint;
    //画折线对应的画笔
    private Paint linePaint, pointPaint, reactPaint;
    private int width;
    private int height;
    //x轴的原点坐标
    ...
    private int yOri;
    //第一个点X的坐标
    private float xInit = -interval * 5;
    //第一个点对应的最大Y坐标
    ...
    private Map<String, String> yString = new HashMap<>();
    //点击的点对应的X轴的第几个点，默认1
    ...
    private void initPaint() {
        xyPaint1 = new Paint();
        xyPaint1.setAntiAlias(true);
        xyPaint1.setStrokeWidth(xylinewidth);
        xyPaint1.setStrokeCap(Paint.Cap.ROUND);
        xyPaint1.setColor(0x26cccccc);
        ...
        pointPaint.setStrokeWidth(xylinewidth);
        ...
        reactPaint.setStrokeWidth(xylinewidth);
        reactPaint.setStrokeCap(Paint.Cap.ROUND);
        reactPaint.setColor(Color.WHITE);
...
//            //X轴文本最大高度
        xValueRect = getTextBounds("今天", xyTextPaint);
        ...
        maxXInit = xInit;
        Logger.d("minXInit" + minXInit);
        Logger.d("maxXInit" + maxXInit);
    }
...
//        super.onDraw(canvas);
    ...
    private void drawBrokenPoint(Canvas canvas) {
        ...
        for (int i = 0; i < xValue.size(); i++) {
            ...
            Log.e("-----------------", "" + xValue.get(i) + "   " + value.get(xValue.get(i)));
            //绘制小椭圆
            if (value.get(xValue.get(i)) <= 11 && value.get(xValue.get(i)) > 0) {
                RectF oval = new RectF(x - 10, yOri - 20 - value.get(xValue.get(i)) / 2, x + 10, yOri - 20 + value.get(xValue.get(i)) / 2);
                canvas.drawOval(oval, reactPaint);
            }
            if (value.get(xValue.get(i)) >= 12) {
                Path path1 = new Path();
                path1.moveTo(x - 10, y);
                path1.lineTo(x - 10, yOri - 20);
                RectF oval = new RectF(x - 10, yOri - 30, x + 10, yOri - 10);
                path1.arcTo(oval, 0.0f, 180.0f);
                path1.lineTo(x + 10, yOri - 20);
                path1.lineTo(x + 10, y);
                RectF oval1 = new RectF(x - 10, y - 10, x + 10, y + 10);
                path1.arcTo(oval1, 0.0f, -180.0f);
                path1.close();
                canvas.drawPath(path1, reactPaint);
            }
...
/*            if(value.get(xValue.get(i))>=30){
            ...
            //canvas.drawCircle(x, y, dp2, pointPaint);

            //绘制选中的点
            if (xString.get(i).equals("现在")) {
                pointPaint.setStyle(Paint.Style.FILL);
                pointPaint.setColor(0x4aFFFFFF);
                //canvas.drawCircle(x, y, dpToPx(9), pointPaint);
                pointPaint.setColor(0xffFFFFFF);
                //canvas.drawCircle(x, y, dpToPx(6), pointPaint);
                pointPaint.setColor(0xff3D9D75);
                //canvas.drawCircle(x, y, dpToPx(2), pointPaint);
            }
            //drawFloatTextBox(canvas, x, y - dp7, changeFloatToText(value.get(xValue.get(i))));
            String text = "";
            if (value.get(xValue.get(i)).toString().equals("0")) {
                text = "";
            } else {
                //text = value.get(xValue.get(i)).toString();
                text = yString.get(xValue.get(i));
            }
            drawFloatTextBox(canvas, x, y - dp7, text);
        }
    }
...
/*             Shader.TileMode三种模式
            ...
            Log.e("&&&&&&", i + "   : " + value.get(xValue.get(i)) + " ys: " + ys + "  ye: " + ye);
            if (ys - ye <= 0.01) {
                return;
            }
            Path path1 = new Path();
            path1.moveTo(xs - 10, ys);
            path1.lineTo(xs - 10, yOri - 10);
            RectF oval = new RectF(xs - 10, yOri - 15, xs + 10, yOri - 5);
            path1.arcTo(oval, 0.0f, 180.0f);
            path1.lineTo(xs + 10, yOri - 10);
            path1.lineTo(xs + 10, ys);
            RectF oval1 = new RectF(xs - 10, ys - 5, xs + 10, ys + 5);
            path1.arcTo(oval1, 0.0f, -180.0f);
            path1.close();
            linePaint.setStyle(Paint.Style.FILL);
        ...
        if (value.get(xValue.get(xValue.size() - 1)) == 0) {
            return;
        }
        Log.e("&&&&&&eee", "   : " + " ys: " + ys + "  ye: " + ye);
        Path path1 = new Path();
        path1.moveTo(xe - 10, ye);
        path1.lineTo(xe - 10, yOri - 10);
        RectF oval = new RectF(xe - 10, yOri - 15, xe + 10, yOri - 5);
        path1.arcTo(oval, 0.0f, 180.0f);
        path1.lineTo(xe + 10, yOri - 10);
        path1.lineTo(xe + 10, ye);
        RectF oval1 = new RectF(xe - 10, ye - 5, xe + 10, ye + 5);
        path1.arcTo(oval1, 0.0f, -180.0f);
        path1.close();
        canvas.drawPath(path1, reactPaint);
...
/*             Shader.TileMode三种模式
            ...
            //canvas.drawRect(0, 0, yLength, yOri + xylinewidth / 2, linePaint);
            Path path1 = new Path();
            path1.moveTo(xs - 10, 0);
            path1.lineTo(xs - 10, ys);
            path1.lineTo(xs + 10, ys);
            path1.lineTo(xs + 10, 0);
...
/*        path.moveTo(xOri - xylinewidth / 2 - dpToPx(5), dpToPx(12));
        ...
        canvas.drawLine(xOri, yOri + xylinewidth / 2 - height / 3, width, yOri + xylinewidth / 2 - height / 3, xyPaint1);
        canvas.drawLine(xOri, yOri + xylinewidth / 2 - height / 3 * 2, width, yOri + xylinewidth / 2 - height / 3 * 2, xyPaint1);
/*        //绘制x轴箭头
        xyPaint.setStyle(Paint.Style.STROKE);
        path = new Path();
        //整个X轴的长度
...
//                } else {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        this.getParent().requestDisallowInterceptTouchEvent(isScrolling);//当该view获得点击事件，就请求父控件不拦截事件
    ...
    public void setValue(Map<String, Integer> value, List<String> xValue, List<Integer> yValue,
                         List<String> xString, List<Integer> yDegreeValue, List<String> yDegreeString,
                         List<Integer> pointColor, Map<String, String> yString) {
        this.value = value;
        this.xValue = xValue;
        this.yValue = yValue;
